...
namespace HVAC_Program
{
    public partial class Drawing : Form
    {
        ...
        public static double HeatOFNorthDoor = 0;
        ...
        private void WinHbtn_Click(object sender, EventArgs e)
        {
            if (Window.Text == "West")
            {
                HeatOFWestWindow = (double.Parse(WinLen.Text) * double.Parse(WinWid.Text) * 27) / 1000;
        ...
        private void PWallHbtn_Click(object sender, EventArgs e)
        {
            ...
            {
                HeatOFNorthP_Wall = (0.4789 * double.Parse(PWallLen.Text) * double.Parse(PWallHeight.Text) * 4) / 1000;
                PWallH_N.Text = HeatOFNorthP_Wall.ToString();
            }
            else if (PartitionWall.Text == "West")
            {
                HeatOFWestP_Wall = (0.4789 * double.Parse(PWallLen.Text) * double.Parse(PWallHeight.Text) * 4) / 1000;
                PWallH_W.Text = HeatOFWestP_Wall.ToString();
            }
            else if (PartitionWall.Text == "East")
            {
                HeatOFSouthP_Wall = (0.4789 * double.Parse(PWallLen.Text) * double.Parse(PWallHeight.Text) * 4) / 1000;
        ...
        private void DoorHeatbtn_Click(object sender, EventArgs e)
        {
            ...
            else if (Door.Text == "North")
            {
                HeatOFNorthDoor = (0.18 * double.Parse(DoorLen.Text) * double.Parse(DoorWidth.Text) * 14) / 1000;
                DoorHeat_N.Text = HeatOFNorthDoor.ToString();
            }
            else
            {
                MessageBox.Show("Please fill the length and Width field", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        {
            TotalRoomHeat = HeatOFNorthWindow + HeatOFWestWindow + HeatOFNorthP_Wall + HeatOFWestP_Wall + HeatOFSouthP_Wall + HeatOFEastEX_Wall + HeatOFRoof + HeatOFEastDoor + HeatOFWestDoor + HeatOFNorthDoor + HeatOfInfl + HeatofLamps + HeatofPeople + HeatofAC + HeatofFans;
