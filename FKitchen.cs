...
namespace HVAC_Program
{
    public partial class FKitchen : Form
    {
        ...
        private void WinHbtn_Click(object sender, EventArgs e)
        {
            if (Window.Text == "East")
            {
                HeatOFNorthWindow = (double.Parse(WinLen.Text) * double.Parse(WinWid.Text) * 397) / 1000;
        ...
        private void PWallHbtn_Click(object sender, EventArgs e)
        {
            ...
            {
                HeatOFNorthP_Wall = (0.4789 * double.Parse(PWallLen.Text) * double.Parse(PWallHeight.Text) * 6) / 1000;
                PWallH_N.Text = HeatOFNorthP_Wall.ToString();
            }
            else if (PartitionWall.Text == "West")
            {
                HeatOFWestP_Wall = (0.4789 * double.Parse(PWallLen.Text) * double.Parse(PWallHeight.Text) * 6) / 1000;
        ...
        private void Ovenbtn_Click(object sender, EventArgs e)
        {
            if (NoOfOven.Text != "")
            {
                HeatofOven = (2500 * double.Parse(NoOfOven.Text)) / 1000;
                OvenHeat.Text = HeatofOven.ToString();
            }
            else
            {
                MessageBox.Show("Please fill the Oven field", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }

        private void Toasterbtn_Click(object sender, EventArgs e)
        {
            if (NoOfToaster.Text != "")
            {
                HeatofToaster = (800 * double.Parse(NoOfToaster.Text)) / 1000;
                ToasterHeat.Text = HeatofToaster.ToString();
            }
            else
            {
                MessageBox.Show("Please fill the Toaster field", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        public double GetFKitchen()
        {
