...
{
    public partial class FBed2 : Form
    {
        ...
        private void WinHbtn_Click(object sender, EventArgs e)
        {
            if (Window.Text == "East")
            {
                HeatOFEastWindow = (double.Parse(WinLen.Text) * double.Parse(WinWid.Text) * 268) / 1000;
        ...
        {
            TotalRoomHeat = HeatOFEastWindow + HeatOFWestWindow + HeatOFNorthP_Wall + HeatOFWestP_Wall + HeatOFSouthP_Wall + HeatOFEastEX_Wall + HeatOFRoof + HeatOFDoor + HeatOfInfl + HeatofLamps + HeatofPeople + HeatofAC + HeatofFans;
            LoadBox.Text = TotalRoomHeat.ToString();
        }
        public double GetFBed2()
        {
